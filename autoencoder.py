...
def full_network(params):
    ...
    x = tf1.placeholder(tf.float32, shape=[None, input_dim], name='x')
    dx = tf1.placeholder(tf.float32, shape=[None, input_dim], name='dx')
    if model_order == 2:
        ddx = tf1.placeholder(tf.float32, shape=[None, input_dim], name='ddx')
    ...
    if params['coefficient_initialization'] == 'xavier':
        sindy_coefficients = tf1.get_variable('sindy_coefficients', shape=[library_dim,latent_dim], initializer=tf.contrib.layers.xavier_initializer())
    elif params['coefficient_initialization'] == 'specified':
        sindy_coefficients = tf1.get_variable('sindy_coefficients', initializer=params['init_coefficients'])
    elif params['coefficient_initialization'] == 'constant':
        sindy_coefficients = tf1.get_variable('sindy_coefficients', shape=[library_dim,latent_dim], initializer=tf.constant_initializer(1.0))
    elif params['coefficient_initialization'] == 'normal':
        sindy_coefficients = tf1.get_variable('sindy_coefficients', shape=[library_dim,latent_dim], initializer=tf.initializers.random_normal())
    
    if params['sequential_thresholding']:
        coefficient_mask = tf1.placeholder(tf.float32, shape=[library_dim,latent_dim], name='coefficient_mask')
...
def build_network_layers(input, input_dim, output_dim, widths, activation, name):
    ...
    for i,n_units in enumerate(widths):
        W = tf1.get_variable(name+'_W'+str(i), shape=[last_width,n_units],
            initializer=tf.keras.initializers.GlorotUniform()) #tf.contrib.layers.xavier_initializer())
        b = tf1.get_variable(name+'_b'+str(i), shape=[n_units],
        ...
        weights.append(W)
        biases.append(b)
    W = tf1.get_variable(name+'_W'+str(len(widths)), shape=[last_width,output_dim],
        initializer=tf.keras.initializers.GlorotUniform()) #initializer=tf1.contrib.layers.xavier_initializer())
    b = tf1.get_variable(name+'_b'+str(len(widths)), shape=[output_dim],
        initializer=tf.constant_initializer(0.0))
    input = tf.matmul(input,W) #+ b remove bias influence, test to see if removes latent vector drift
    weights.append(W)
    biases.append(b)
...
def z_derivative(input, dx, weights, biases, activation='elu'):
    ...
    if activation == 'elu':
        ...
        for i in range(len(weights)-1):
            input = tf.matmul(input, weights[i]) + biases[i]
            dz = tf.multiply(tf1.to_float(input>0), tf.matmul(dz, weights[i]))
