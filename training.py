...
def train_network(training_data, val_data, params):
    ...
    with tf.Session() as sess:
        ...
        return results_dict
"""  
def run_encoder(training_data, params):
    
    saver = tf.train.import_meta_graph('/Users/wilsobe/Desktop/Python Files/Real Data/SindyAutoencoders-master/src/500 Epoch 2nd order (7 vars)/TSC_Seizures_2023_08_16_15_24_40_179965.meta')
    
    with tf.Session() as sess:
        model = saver.restore(sess, tf.train.latest_checkpoint('./'))
        sess.run(tf.global_variables_initializer())
    
        for j in range(params['epoch_size']//params['batch_size']):
            batch_idxs = np.arange(j*params['batch_size'], (j+1)*params['batch_size'])
            train_dict = create_feed_dictionary(training_data, params, idxs=batch_idxs)
            condensed = sess.run(feed_dict=train_dict)
    return condensed
"""
